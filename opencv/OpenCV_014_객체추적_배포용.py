{
 "cells": [
  ...
  },
  {
   ...
   "source": [
    "- 히스토그램 역투영\n",
    "    - 사용자가 선택한 이미지 영역의 색상 히스토그램을 분석해서 분석한 결과의 색상분포와 유사한 이미지 영역을\n",
    "      원본 이미지에서 찾아서 (히스토그램 역투영) 표시하는것\n",
    "      \n",
    "     <img src=\"./image/14_histogram_proj.png\" width=70%>"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "cap = cv2.VideoCapture('./image/dahyechom.webm')\n",
    "while cap.isOpened():\n",
    "    ret, frame = cap.read()\n",
    "    if not ret:\n",
    "        print('비디오 읽기 실패')\n",
